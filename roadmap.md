...
> **Version:** 1.3 · **Date:** 2026-09-11 · **Status:** Planning
...
Every significant decision (§1.3), including dual auth (Clerk + API keys) and Resend email, gets a one-page **ADR** in `docs/adr/` — context, options, choice, consequences — so sibling-platform teams and future maintainers get the *why*, not just the *what*.
...
| **1 — Ingestion** | 2–3 | MT5 live + historical, trading calendar, news, macro, registry, watchdog, **restore drill** | 2+ years M1 backfilled; live bars flowing to staging; watchdog alerts on killed feed; backup restore proven |
...
| Day | Work |
|-----|------|
| W4·D1 | `analytics/metrics.py` part 1: realized vol, ATR + percentiles, rolling returns per timeframe (M1→H1→D1 via aggregates), session-aware via calendar module. |
| W4·D2 | Metrics part 2 (split file if >300 lines): trend strength, rolling correlation matrix, spread/liquidity stats. |
| W4·D3 | `analytics/regimes.py`: rule-based classifier (vol percentile × trend strength → 4 regimes + probabilities). |
| W4·D4 | Backtest classifier over history; tune thresholds; measure flip-rate noise budget. |
| W4·D5 | Regression fixtures pinning classifier behavior; unit tests for all metrics. Status note. |
| W5·D1 | `analytics/profiles.py`: daily profile builder assembling `AssetProfile` per contract. |
| W5·D2 | Event-triggered profile refresh; profile versioning + history retention (migration). |
| W5·D3 | Forecast/regime-call immutable log writer; every classification recorded with model version. |
| W5·D4 | `scheduler.py` as **worker entrypoint**: APScheduler wiring (metrics cadence, daily profiles, log writes). Compose `--profile worker`. **Deploy worker as second Railway service (same image, worker entrypoint).** Full pipeline dry-run on staging. |
| W5·D5 | HMM classifier spike (`hmmlearn`) as v2 candidate — compare vs rule-based; decide & write ADR. Tag `v0.3-analytics`. |
...
| Regime classifier noise (excessive flips) | Flip-rate budget measured in W4·D4; hysteresis thresholds; HMM candidate evaluated before promotion |
